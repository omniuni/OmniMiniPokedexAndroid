...
object UtilityLoaderOld: IOnApiLoad {
	...
	fun enqueue(map: Map<IApi, String>){
		...
		map.forEach { apiEntry ->
			...
			Log.d(UtilityLoaderOld::class.simpleName, "Check ID: $id")
			if(mToLoad.containsKey(id)) return
			Log.d(UtilityLoaderOld::class.simpleName, "Enqueue ID: $id")
	...
	private fun continueLoading(){
		...
		Log.d(UtilityLoaderOld::class.simpleName, "Loading: $toLoadPairKey")
	...
	override fun onSuccess(success: IApi) {
		Log.d(UtilityLoaderOld::class.simpleName, "Remove: ${success.getUrl()}")
		mToLoad.remove(success.getUrl())
		Log.d(UtilityLoaderOld::class.simpleName, "Success: ${success.getUrl()}")
		mListeners.forEach {
	...
	override fun onFailed(failure: IApi) {
		Log.d(UtilityLoaderOld::class.simpleName, "Remove: ${failure.getUrl()}")
		mToLoad.remove(failure.getUrl())
		Log.d(UtilityLoaderOld::class.simpleName, "Failure: ${failure.getUrl()}")
		mListeners.forEach {
			it.onFailed(failure)
		}
		continueLoading()
